...
class Client extends WeatherClient {
    getInfos(maxForecasts, intervalHour, dailyForecast) {
        const infos = [];
        ...
        for (let i = 0; i < forecasts.length; i++) {
            ...
            if (dailyForecast) {
                const day = datetime.get_day_of_year();
                if (day === lastDay) continue; // Already got a forecast for this day
                lastDay = day;
            } else {
...
class Header extends St.BoxLayout {
    ...
    _init(client, options) {
        ...
        this._locationLabel = new St.Label({
            style_class: "beQS-location-label",
            y_align: Clutter.ActorAlign.CENTER,
            opacity: 190,
        });
    ...
    _sync() {
        if (this._locationLabel.visible =
...
class WeatherSection extends St.Button {
    ...
    _addForecast(forecast, col) {
        const [, timestamp] = forecast.get_value_update();
        const date = new Date(timestamp * 1000);
        
        let timeStr;
        if (this._options.dailyForecast) {
    ...
    _updateForecasts() {
        const infos = this._client.getInfos(this._options.maxForecasts, this._options.intervalHour, this._options.dailyForecast);
        // RTL support
...
class WeatherWidget extends St.BoxLayout {
    ...
    _init(options) {
        ...
        // Sync changes
        this._client.connectObject("changed", this._updateStyleClass.bind(this), this);
        this._updateStyleClass();
    }
    _updateStyleClass() {
        const options = this._options;
        let style = "beQS-weather";
        if (options.removeShadow)
            style += " beQS-weather-remove-shadow";
        if (options.compact)
            style += " beQS-weather-compact";
...
export class WeatherWidgetFeature extends FeatureBase {
    loadSettings(loader) {
        ...
        this.dailyForecast = loader.loadBoolean("weather-daily-forecast");
        this.hygrometerEnabled = loader.loadBoolean("system-hygrometer-enabled");
    }
    reload(key) {
        switch (key) {
            ...
            case "weather-remove-shadow":
                ...
                break;
            case "weather-show-header":
                if (!this.enabled || !this.weatherWidget) return;
                this.weatherWidget._header.visible = this.header;
                break;
            case "weather-click-command":
                break;
            case "weather-daily-forecast":
            case "weather-interval-hour":
            case "weather-max-forecasts":
                if (!this.enabled || !this.weatherWidget) return;
                this.weatherWidget._client.update(); // Re-fetch logic
                this.weatherWidget._item._sync(); // Force redraw of forecasts
                break;
            default:
                super.reload();
                break;
    ...
    onLoad() {
        if (this.enabled) {
            this.maid.destroyJob(this.weatherWidget = new WeatherWidget(this));
            Global.QuickSettingsGrid.add_child(this.weatherWidget);
            Global.QuickSettingsGrid.layout_manager.child_set_property(Global.QuickSettingsGrid, this.weatherWidget, "column-span", 2);
        }
        
        if (this.hygrometerEnabled) {
            this.maid.destroyJob(this.hygrometerWidget = new HygrometerWidget(this));
            Global.QuickSettingsGrid.add_child(this.hygrometerWidget);
            Global.QuickSettingsGrid.layout_manager.child_set_property(Global.QuickSettingsGrid, this.hygrometerWidget, "column-span", 2);
        }
    }
    onUnload() {
        this.weatherWidget = null;
        this.hygrometerWidget = null;
    }
}

// #region Hygrometer
class HygrometerWidget extends St.BoxLayout {
    constructor(options) {
        super({
            style_class: "beQS-hygrometer message",
            vertical: false,
            x_expand: true,
        });
        this._options = options;
        ...
        const icon = new St.Icon({
            icon_name: 'weather-humidity-symbolic',
            style_class: 'hygrometer-icon',
        });
        this.add_child(icon);
        ...
        this._label = new St.Label({
            text: '--%',
            y_align: Clutter.ActorAlign.CENTER,
            style_class: 'hygrometer-label',
        });
        this.add_child(this._label);

        this._client.connectObject('changed', this._update.bind(this), this);
        this._update();
    }

    _update() {
        if (!this._client.available || !this._client.info.is_valid()) {
            this._label.text = '--%';
            return;
        }
        const humidity = this._client.info.get_value_humidity();
        this._label.text = `${Math.round(humidity)}%`;
    }
}
GObject.registerClass(HygrometerWidget);
// #endregion Hygrometer
