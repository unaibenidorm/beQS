...
class StreamSlider extends QuickSlider {
    ...
    _init(control, stream, options) {
        ...
        // Icon click -> mute/unmute fiable
        this.iconReactive = true;
        this.connect("icon-clicked", () => {
            if (!this._stream)
                return;

            const newMuted = !this._stream.is_muted;

            this._stream.change_is_muted(newMuted);

            // Si desmuteas y el volumen está a 0, súbelo al 50 %
            if (!newMuted && this._stream.volume === 0) {
                this._stream.volume = this._control.get_vol_max_norm() * 0.5;
        ...
        this._inDrag = false;
        this._sliderChangedId = this._maid.connectJob(
            this.slider,
            "notify::value",
            this._sliderChanged.bind(this)
        );
        this._maid.connectJob(this.slider, "drag-begin", () => {
            this._inDrag = true;
        });
        this._maid.connectJob(this.slider, "drag-end", () => {
            this._inDrag = false;
        });
    ...
    _updateAllowAmplified() {
        ...
        this.slider.maximumValue = this._allowAmplified
            ? this.getMaxLevel()
            : 1;
        if (this._stream)
            this._updateSlider();
...
class VolumeMixerItem extends St.BoxLayout {
    ...
    _getStreamId() {
        const appId =
            this._stream.get_application_id?.() || this._stream.application_id;
        const name = this._stream.get_name?.() || this._stream.name;
    ...
    _sync() {
        const hiddenDevices = this._options.hiddenDevices || [];
        const streamId = this._getStreamId();

        if (streamId && hiddenDevices.includes(streamId)) {
            this.hide();
        } else {
            this.show();
            this._slider._sync();
...
class VolumeMixerList extends St.BoxLayout {
    constructor(options) {
        ...
        this._maid.connectJob(
            Global.Settings,
            "changed::volume-mixer-hide-devices",
            () => this._syncVisibility()
        );
        this._maid.connectJob(
            Global.Settings,
            "changed::volume-mixer-custom-names",
            () => this._syncVisibility()
        );
...
class VolumeMixerWidget extends St.BoxLayout {
    ...
    _updateMaxHeight() {
        ...
        this.style = maxHeight ? `max-height:${maxHeight}px;` : "";
    }

    _updateStyleClass() {
        this.style_class = "beQS-volume-mixer";
    }
    ...
    vfunc_get_preferred_height(for_width) {
        ...
        return [
            Math.min(maxHeight, contentHeight[0]),
            Math.min(maxHeight, contentHeight[1]),
        ];
    }
...
export class VolumeMixerWidgetFeature extends FeatureBase {
    loadSettings(loader) {
        ...
        // showIcon ya no se usa: icono siempre visible
        this.showIcon      = true;
        this.maxHeight     = loader.loadInt("volume-mixer-max-height");

        // labelText forzado internamente a "none"
        this.labelText     = "none";
        ...
        this.hiddenDevices = loader.loadArray("volume-mixer-hide-devices") || [];
        this.customNames   = loader.loadJSON("volume-mixer-custom-names") || {};
    }
    ...
    createMenu(slider) {
        ...
        this.mixerMenuButton = new St.Button({
            ...
            y_expand: true,
            visible: true,
            accessible_name: _("Open volume mixer"),
        });
    ...
    onLoad() {
        ...
        if (this.menuEnabled) {
            Global.GetStreamSlider()
                .then(({ OutputStreamSlider }) => this.createMenu(OutputStreamSlider))
                .catch(Logger.error);
        } else {
    ...
    reload(key) {
        switch (key) {
            ...
            case "volume-mixer-show-scrollbar":
                ...
                break;
            case "volume-mixer-hide-devices":
            case "volume-mixer-custom-names":
                if (!this.enabled) return;
                this.volumeMixerWidget._list._syncVisibility();
                break;
            default:
                super.reload();
                break;
