...
export class SystemIndicatorLayoutFeature extends FeatureBase {
    ...
    reload(key) {
        if (key.startsWith("system-indicator-")) {
            this.onUpdate();
            return;
        }
        super.reload(key);
    ...
    onUpdate() {
        ...
        const head = [];
        const indicators = children.filter(child => child instanceof SystemIndicator);
        const middle = [...indicators];
        const tail = [];
        let overNonOrdered = false;

        // Apply hide/show for all indicators
        ...
        for (const item of [head, middle, tail].flat()) {
            if (last)
                ...
                Global.Indicators.set_child_at_index(item, 0);
            last = item;
        }
    }
    onLoad() {
        ...
        if (this.privacyIndicatorStyle == "accent") {
            privacyIndicatorStyle.add("beQS-privacy-indicator-use-accent");
        }
        else if (this.privacyIndicatorStyle == "monochrome") {
            privacyIndicatorStyle.add("beQS-privacy-indicator-use-monochrome");
        }
            ...
            this.maid.functionJob(() => {
                Global.Indicators.style_class =
                    new StyleClass(Global.Indicators.style_class)
                        .remove("beQS-privacy-indicator-use-accent")
                        .remove("beQS-privacy-indicator-use-monochrome")
                        .stringify();
            });
        }
        ...
        if (this.accentScreenSharingIndicator) {
            Main.panel.statusArea["screenSharing"].style_class =
                new StyleClass(Main.panel.statusArea["screenSharing"].style_class)
                    .add("beQS-screen-sharing-indicator-use-accent")
                    .stringify();
            this.maid.functionJob(() => {
                Main.panel.statusArea["screenSharing"].style_class =
                    new StyleClass(Main.panel.statusArea["screenSharing"].style_class)
                        .remove("beQS-screen-sharing-indicator-use-accent")
                        .stringify();
            });
        }
        ...
        if (this.accentScreenRecordingIndicator) {
            Main.panel.statusArea["screenRecording"].style_class =
                new StyleClass(Main.panel.statusArea["screenRecording"].style_class)
                    .add("beQS-screen-recording-indicator-use-accent")
                    .stringify();
            this.maid.functionJob(() => {
                Main.panel.statusArea["screenRecording"].style_class =
                    new StyleClass(Main.panel.statusArea["screenRecording"].style_class)
                        .remove("beQS-screen-recording-indicator-use-accent")
                        .stringify();
            });
        }
        // Ordering
