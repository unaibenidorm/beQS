...
export class SystemItemsLayoutFeature extends FeatureBase {
    loadSettings(loader) {
        ...
        this.showBatteryPercentage = loader.loadBoolean("system-battery-show-percentage");
    ...
    onLoad() {
        ...
        this.getItmes().then(items => {
            ...
            if (this.showBatteryPercentage && items.battery) {
                 // Try to force percentage visibility
                 // In GNOME 45+, powerToggle has a _percentageLabel
                 if (items.battery._percentageLabel) {
                     this.maid.connectJob(items.battery, 'notify::visible', () => {
                         items.battery._percentageLabel.visible = true;
                     });
                     items.battery._percentageLabel.visible = true;
                 }
            }
